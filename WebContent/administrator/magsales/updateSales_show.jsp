<%@ page language="java" contentType="text/html; charset=utf-8"
  ...
  <%@ page import="java.sql.*,java.util.List,Util.DBConnection,JPA.SalespersonDao,model.Salesperson" %>
...
<div class="content">
	...
	<h2>修改销售员信息</h2>
		<div class="antzoen">
		<c:set var="salesid" value="${param.id}"></c:set>
		<c:set var="cd" value="${SalespersonDao()}"></c:set>
		<c:set var="ci" value="${cd.find(salesid)}"></c:set>
	
		<form action="saveSales_do.jsp" method="post">
		<table>
				<tr>
					<td>销售员工号：</td>
					<td><input type="text" name="salesid" value="${ci.getSalespersonid()}${c1.getSalespersonid()}"/><span class="error">${errors.get("salespersonid")}</span></td>
				</tr>
				<tr>
					<td>销售员名字：</td>
					<td><input type="text" name="salesname" value="${ci.getSalesname()}${c1.getSalesname()}"/><span class="error">${errors.get("salesname")}</span></td>
				</tr>
				<tr>
					<td>销售员年龄：</td>
					<td><input type="text" name="salesage" value="${ci.getSalesage()}${c1.getSalesage()}"/><span class="error">${errors.get("salesage")}</span></td>
				</tr>
				<tr>
					<td>性别：</td>
					<td><input type="text" name="salessex" value="${ci.getSalessex()}${c1.getSalessex()}"/><span class="error">${errors.get("salessex")}</span></td>
				</tr>
				<tr>
					<td>出生日期：</td>
					<td><input type="date" name="birthday" value="${ci.getSalesbirthday()}${c1.getSalesbirthday()}"/><span class="error">${errors.get("salesbirthday")}</span></td>
				</tr>
				<tr>
					<td>用户名：</td>
					<td><input type="text" name="account" value="${ci.getAccountname()}${c1.getAccountname()}"/><span class="error">${errors.get("accountname")}</span></td>
				</tr>
			</table>
			<div class="button">
